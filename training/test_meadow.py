...
def _load_dirs():
  ...
  files = [(f, 0.0) for f in negative_list[:800]] + [(f, 1.0) for f in positive_list[:800]]
  random.shuffle(files)
  train_files = files[:1280]
  test_files = files[1280:]
...
train_dataset = train_dataset.map(partial(_parse_function, sx=256, sy=256)).map(_vector).cache().batch(40).repeat()
...
test_dataset = test_dataset.map(partial(_parse_function, sx=256, sy=256)).map(_vector).cache().batch(8).repeat()


callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=50),
             tf.keras.callbacks.ModelCheckpoint(filepath='meadow.c3.best.h5', monitor='val_loss', save_best_only=True)]
...
print(good / float(test_steps * 8))
print(good_1 / float(test_steps * 8))
...
tf.keras.models.save_model(model1, '/home/gsvigruha/meadow.model.c3.h5', include_optimizer=False)
